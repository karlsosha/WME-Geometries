...
`;
    ...
    const colorList: Set<string> = new Set(["deepskyblue", "magenta", "limegreen", "orange", "teal", "navy", "maroon"]);
    ...
    let geolist: HTMLUListElement;

    function loadLayers() {
        // Parse any locally stored layer objects
        const files: Record<string, File> = JSON.parse(localStorage.getItem("WMEGeoLayers") || "[]");
        for (const f in files) processGeometryFile(files[f]);
    }

    const geobox: HTMLDivElement = document.createElement<"div">("div");

    function appendGeoBox(sidepanel: HTMLElement | null | undefined) {
        if (sidepanel && !document.contains(geobox)) {
            sidepanel.append(geobox);
        }
    }

    function triggerOnElementUpdate(selector: string, waitToExist: boolean = false, root: ShadowRoot | null | undefined = undefined) {
    ...
    function init() {
        ...
        const geoform: HTMLFormElement = document.createElement<"form">("form");
        geobox.appendChild(geoform);

        const inputfile: HTMLInputElement = document.createElement<"input">("input");
        inputfile.type = "file";
    ...
    function drawStateBoundary() {
        ...
        for(const f of features) {
            ...
            if (f.properties && !f.properties?.style) f.properties.style = layerStyle;
            if (!f.id) {
    ...
    function addGeometryLayer() {
        ...
        const fileListElement: HTMLInputElement = document.getElementById("GeometryFile") as HTMLInputElement;
        const fileList: FileList | null = fileListElement.files;
        if (!fileList) return;
    ...
    function processGeometryFile(file: File) {
        ...
        const filename: string | undefined = file?.name?.replace(`.${fileext}`, "");
        if (!file || !file?.name || !fileext || !filename) return;
        fileext = fileext ? fileext.toUpperCase() : "";

        // add list item
        const color: string | undefined = colorList.values().next().value;
        if (!color) {
            console.error("Cannot add Any more Layers at this point");
            return;
        }
        ...
        reader.onload = ((theFile: File) => (ev: ProgressEvent<FileReader>) => {
            if (!color) {
            ...
            parseFile(tObj);
            const filenames: Record<string, File> = JSON.parse(localStorage.getItem("WMEGeoLayers") || "[]");
    ...
    function polygonSanitization(f: GeoJSON.Feature<Polygon>): GeoJSON.Feature<Polygon> {
        ...
        const resPolygonCoordinates: Position[][] = [];
        for (const poly of f.geometry.coordinates) {
            const resSubPolyCoordinates: Position[] = [];
            const positionMap: Map<string, number[]> = new Map<string, number[]>();
            for (let ix = 0; ix < poly.length - 1; ++ix) {
                const key: string = poly[ix].toString();
            ...
            positionMap.forEach((value: number[], key: string, map: Map<string, number[]>) => {
                if (value.length > 1) {
                    if (value.length % 2 !== 0) {
    ...
    function remove3DPoints(
        ...
        switch (feature.geometry.type) {
            case "Point":
                ...
                break;
            case "LineString": {
                const lsPos: Position[] = feature.geometry.coordinates.map((pos: Position) =>
            ...
            case "Polygon": {
                const polyPos: Position[][] = feature.geometry.coordinates.map((poly) =>
    ...
    function sanityCheck(source: GeoJSON.Feature[] | undefined): GeoJSON.Feature[] | undefined {
        if (!source) return undefined;
        const resFeatures: GeoJSON.Feature[] | undefined = [];
    ...
    function parseFile(layerObj: LayerStoreObj) {
        ...
        if (features.length > 0) {
            ...
            for (const attrib in features[0].properties) {
                ...
                $(inputElement).on("change", (event: JQuery.TriggeredEvent) => {
        ...
        if (features.length === 0) {
            ...
            $(".clear-layer-button").on("click", function () {
                ...
                const listId: string | undefined = this.textContent?.replace("Clear ", "");
                if (!listId) return;
